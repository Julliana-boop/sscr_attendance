...
function updateStatus(subject, section, studentId, selectEl) {
    ...
    const rec = DB.get(subject, section, studentId);
    let timeIn = rec.timeIn || '';
    if (newStatus === 'Present' || newStatus === 'Late') {
        if (!timeIn) timeIn = new Date().toLocaleTimeString('en-US', { hour:'2-digit', minute:'2-digit' });
    } else if (newStatus === 'Absent') {
        timeIn = '';
    }
    // Persist through DB so localStorage is updated immediately
    DB.set(subject, section, studentId, newStatus, timeIn);
