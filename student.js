// ═══════════════════════════════════════════════════════════
//  SSC-R Attendance System — Student Dashboard Logic
//  Supports: MALACAS (0031), TULABING (0032), ESPIRITU (0029)
//  All other students fall back to a generic empty profile.
// ═══════════════════════════════════════════════════════════

// ── Per-student attendance records (4 subjects × 28 days Feb 2026) ──
...
const STUDENT_PROFILES = {
    ...
    '2026-0032': {
        id:      '2026-0032',
        name:    'Jhon Raven Tulabing',
        display: 'TULABING, Jhon Raven',
        grade:   'Grade 7',
        section: 'Section B',
        raw: {
            Math:    ['P','P','A','P','P','P','L','P','P','P','P','P','A','P','P','P','P','L','P','P','P','P','P','P','P','L','P','P'],
            English: ['P','P','P','P','L','P','P','A','P','P','P','P','P','P','L','P','P','P','A','P','P','P','P','P','P','P','P','P'],
            Science: ['P','P','P','P','P','P','P','P','P','L','P','P','P','P','P','P','P','P','P','P','A','P','P','P','P','P','P','P'],
            Filipino:['P','P','P','P','P','L','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','L','P','P','P','P','P'],
        },
        notifications: [
            { id:1, unread:true,  type:'success', text:'Your attendance for <strong>Science — Feb 21</strong> has been recorded as Present.', time:'Today' },
            { id:2, unread:true,  type:'warn',    text:'You were marked <strong>Absent</strong> in English on <strong>Feb 08, 2026</strong>.', time:'Feb 8' },
            { id:3, unread:false, type:'info',    text:'Monthly attendance report for <strong>February 2026</strong> is now available.', time:'Feb 1' },
        ],
    },

    '2026-0029': {
        id:      '2026-0029',
        name:    'Iubilaeum Espiritu',
        display: 'ESPIRITU, Iubilaeum C.',
        grade:   'Grade 7',
        section: 'Section B',
        raw: {
            Math:    ['A','P','P','P','P','P','P','P','L','P','P','P','P','P','P','A','P','P','P','L','P','P','P','P','P','P','P','P'],
            English: ['P','P','P','P','P','P','P','P','P','P','L','P','P','P','P','P','A','P','P','P','P','L','P','P','P','P','P','P'],
            Science: ['P','P','P','L','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P','P'],
            Filipino:['P','P','P','P','P','P','P','P','P','P','P','P','A','P','P','P','P','P','P','P','P','P','P','P','P','P','P','L'],
        },
        notifications: [
            { id:1, unread:true,  type:'info',    text:'<strong>QR attendance</strong> for Math — Sec B has been opened. Scan now to mark attendance.', time:'Today' },
            { id:2, unread:true,  type:'warn',    text:'You were marked <strong>Absent</strong> in Math on <strong>Feb 01, 2026</strong>.', time:'Feb 1' },
            { id:3, unread:false, type:'success', text:'Your attendance record for <strong>Science</strong> this month is perfect!', time:'Feb 14' },
            { id:4, unread:false, type:'info',    text:'Monthly attendance report for <strong>February 2026</strong> is now available.', time:'Feb 1' },
        ],
    },
};
...
function resolveStudent() {
    ...
    // username format is "S2026-0031", strip the "S" prefix
    const rawId = session.username ? session.username.replace(/^S/, '') : null;
    if (rawId && STUDENT_PROFILES[rawId]) {
        return STUDENT_PROFILES[rawId];
    }
    // Fallback: generic empty profile so the page doesn't crash
    return {
        id:      rawId || 'UNKNOWN',
        name:    session.displayName ? session.displayName.split(',').reverse().join(' ').trim() : 'Student',
        display: session.displayName || 'Unknown Student',
        grade:   'Grade 7',
        section: 'Section B',
        raw: { Math: [], English: [], Science: [], Filipino: [] },
        notifications: [],
    };
}

// ── Active student for this session ──
const STUDENT_DATA = resolveStudent();
const STUDENT      = { id: STUDENT_DATA.id, name: STUDENT_DATA.name, grade: STUDENT_DATA.grade, section: STUDENT_DATA.section };
const RAW_ATTENDANCE = STUDENT_DATA.raw;
...
for (const [subj, days] of Object.entries(RAW_ATTENDANCE)) {
    ...
    const total = p + l + a;
    SUBJECT_STATS[subj] = {
        present: p, late: l, absent: a, total: total || 0,
        rate: total ? Math.round((p + l) / total * 1000) / 10 : 0
    };
...
const OVERALL_RATE   = GRAND_TOTAL ? Math.round(GRAND_ATTENDED / GRAND_TOTAL * 1000) / 10 : 0;
...
for (let day = 28; day >= 1; day--) {
    ...
    for (const subj of ['Math','English','Science','Filipino']) {
        const status = RAW_ATTENDANCE[subj][day - 1] || '';
        if (status) ATTENDANCE_RECORDS.push({ subject: subj, day, date: dateLabel, dateKey, weekday, status });
    }
}
...
for (let day = 1; day <= 28; day++) {
    ...
    const statuses = Object.values(RAW_ATTENDANCE).map(arr => arr[day - 1]).filter(Boolean);
    if (statuses.length) {
...
// ═══════════════════════════════════════════════════════════
//  ON PAGE LOAD — patch the profile UI to show this student
// ═══════════════════════════════════════════════════════════
document.addEventListener('DOMContentLoaded', () => {
    // Build initials from name
    const nameParts = STUDENT_DATA.name.split(' ').filter(Boolean);
    const initials  = nameParts.slice(0, 2).map(w => w[0].toUpperCase()).join('');

    // Patch header profile pill
    const avatarEl = document.querySelector('.student-avatar');
    const nameEl   = document.querySelector('.student-name');
    const roleEl   = document.querySelector('.student-role');
    if (avatarEl) avatarEl.textContent = initials;
    if (nameEl)   nameEl.textContent   = STUDENT_DATA.display;
    if (roleEl)   roleEl.textContent   = `${STUDENT_DATA.id} · ${STUDENT_DATA.section}`;

    // Patch settings modal pre-fill
    ...
    if (settingsNameEl)    settingsNameEl.value    = STUDENT_DATA.display;
    if (settingsSectionEl) settingsSectionEl.value = STUDENT_DATA.section;
    if (settingsAvatarEl)  settingsAvatarEl.textContent = initials;
...
function downloadReport(format) {
    if (format === 'csv') {
        ...
        for (const r of ATTENDANCE_RECORDS) {
            const label = r.status === 'P' ? 'Present' : r.status === 'L' ? 'Late' : 'Absent';
            rows.push([STUDENT.id, STUDENT_DATA.display, r.subject, r.date, r.weekday, label]);
        }
...
function logout() {
    if (confirm('Are you sure you want to logout?')) {
        sessionStorage.removeItem('sscr_user');
        window.location.href = 'login.html';
    }
}

// ═══════════════════════════════════════════════════════════
//  NOTIFICATIONS — loaded from student's own profile
// ═══════════════════════════════════════════════════════════
let _notifications = [...(STUDENT_DATA.notifications || [])];
